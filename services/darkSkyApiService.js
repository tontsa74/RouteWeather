...
export const getRouteWeather = (routes, weathers, startTime) => {
    return async dispatch => {
        ...
        //let firstTime = new Date((startTime + weatherTimeStep) * 1000).setMinutes(0, 0, 0) / 1000;
        ...
        routes.forEach((route, index) => {
            ...
            if (routeIndex == 0) {
                ...
                dispatch(
                    getWeather(routeIndex, latitude, longitude, firstTime)
                );
            }
            ...
            route.legs[0].steps.forEach((step, index) => {
                ...
                while (
                    ...
                    route.legs[0].duration.value - totalDuration >
                        weatherTimeStep
                ) {
                    ...
                    let latitude =
                        (step.end_location.lat - stepStartLat) * ratio +
                        stepStartLat;
                    let longitude =
                        (step.end_location.lng - stepStartLng) * ratio +
                        stepStartLng;
                    // UTC time of weather
...
const findWeather = (latitude, longitude, time) => {
    ...
    tempWeathers.forEach(weather => {
        if (
            ...
            Math.abs(longitude - weather.coord.longitude) <
                weatherLngAccuracy &&
            ...
            if (!result) {
                bestWeather = weather;
                result = true;
            } else if (weather.timestamp > bestWeather.timestamp) {
                bestWeather = weather;
            }
    ...
    if (!result) {
        let weather = new Weather(0, new Coord(latitude, longitude), time);
        tempWeathers.push(weather);
    } else {
        bestWeather.isVisible = true;
    }
